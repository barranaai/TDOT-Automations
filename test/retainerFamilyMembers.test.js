...
test('parseSelections whitelists family member types and coerces accompanying', () => {
  const sel = parseSelections({
    ...
    familyMembers: [
      { type: 'Spouse', name: ' Jane ', accompanying: true, dateOfBirth: '1990-05-01', currentStatus: 'Visitor', countryOfResidence: 'Canada' },
  ...
  assert.deepEqual(sel.familyMembers, [
    { type: 'Spouse', name: 'Jane', dateOfBirth: '1990-05-01', currentStatus: 'Visitor', countryOfResidence: 'Canada', accompanying: true },
    { type: 'Dependent Child', name: 'Kid', dateOfBirth: '', currentStatus: '', countryOfResidence: '', accompanying: true },
  ]);
});
...
test('planMembersFromConsultant materialises only accompanying members with board keys', () => {
  const rows = planMembersFromConsultant({ retainerFamilyMembers: JSON.stringify([
    { type: 'Spouse', name: 'Jane Doe', accompanying: true, dateOfBirth: '1988-03-14', currentStatus: 'Worker', countryOfResidence: 'Canada' },
  ...
  assert.deepEqual(rows, [
    { memberType: 'Spouse', name: 'Jane Doe', memberKey: 'spouse', dateOfBirth: '1988-03-14', currentStatus: 'Worker', countryOfResidence: 'Canada' },
    { memberType: 'Dependent Child', name: 'Kid A', memberKey: 'child-1', dateOfBirth: '', currentStatus: '', countryOfResidence: '' },
    { memberType: 'Parent', name: 'Mom', memberKey: 'parent-1', dateOfBirth: '', currentStatus: '', countryOfResidence: '' },
  ]);
});
...
test('planMembersFromConsultant: blank name gets a placeholder, multi children indexed', () => {
  ...
  assert.deepEqual(rows, [
    { memberType: 'Spouse', name: 'Spouse (consultant-set)', memberKey: 'spouse', dateOfBirth: '', currentStatus: '', countryOfResidence: '' },
    { memberType: 'Dependent Child', name: 'A', memberKey: 'child-1', dateOfBirth: '', currentStatus: '', countryOfResidence: '' },
    { memberType: 'Dependent Child', name: 'B', memberKey: 'child-2', dateOfBirth: '', currentStatus: '', countryOfResidence: '' },
  ]);
});
