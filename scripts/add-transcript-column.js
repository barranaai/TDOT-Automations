/**
 * One-off — add the "Consultation Transcript" link column to the Lead Board so
 * the Teams meeting transcript (fetched via Graph onlineMeetings/transcripts and
 * stored in the lead's OneDrive folder) has a home next to the recording link.
 *
 * INERT: the /webhook/lead automation does not branch on this column.
 *
 *   node scripts/add-transcript-column.js            # dry-run
 *   node scripts/add-transcript-column.js --write    # create it
 */
...
const NEW_COLUMNS = [
  { key: 'transcriptLink', title: 'Consultation Transcript', type: 'link' },
];
...
async function main() {
  ...
  if (!todo.length) { console.log(`"transcriptLink" already present (${existing.transcriptLink}). Nothing to do.`); return; }
