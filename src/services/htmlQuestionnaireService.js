...
const WRAP_CELLS_JS = [
  ...
  '    /* A hidden textarea measures 0 — most forms open as collapsed accordions,',
  '       so size cells again whenever a section/tab is revealed: any click',
  '       (accordion headers, tabs, buttons) re-runs autosize after the toggle. */',
  '    document.addEventListener("click", function () { setTimeout(function () { autosizeAllCells(); }, 50); });',
  '    if (window.IntersectionObserver) {',
  '      var io = new IntersectionObserver(function (entries) {',
  '        for (var i = 0; i < entries.length; i++) if (entries[i].isIntersecting) autosizeCell(entries[i].target);',
  '      });',
  '      var watch = function () { document.querySelectorAll("textarea.tdot-wrap-cell:not([data-tdot-io])").forEach(function (t) { t.setAttribute("data-tdot-io", "1"); io.observe(t); }); };',
  '      watch();',
  '      document.addEventListener("click", function () { setTimeout(watch, 60); });',
  '    }',
...
(function () {
  ...
  function createReviewBar() {
    ...
    document.getElementById('tdot-export-btn').onclick = function () {
      /* multi-member review shows every member → export the whole case; carry the admin key if this tab has one */
      var url = '/q/' + encodeURIComponent(CASE_REF) + '/export-pdf';
      var params = [];
      if (!IS_MULTI_REVIEW) params.push('formKey=' + encodeURIComponent(FORM_KEY));
      var adminKey = new URLSearchParams(location.search).get('key');
      if (adminKey) params.push('key=' + encodeURIComponent(adminKey));
      window.open(url + (params.length ? '?' + params.join('&') : ''), '_blank');
    };
