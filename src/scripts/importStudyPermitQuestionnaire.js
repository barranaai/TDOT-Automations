/**
 * Bulk import Study Permit – Inside & Outside Canada questionnaire (April 2025)
 * into the "Study Permit" group on the Questionnaire Template Board.
 *
 * Single applicant type. One unified questionnaire covers both Inside and
 * Outside Canada (no differences between the two).
 *
 * Usage: node src/scripts/importStudyPermitQuestionnaire.js
 */
...
const TARGET_GROUPS = [
  { groupId: 'group_mm123x60', caseType: 'Study Permit', prefix: 'SP' },
];
...
const QUESTIONS = [
  ...
  { name: 'Employment – Start Date (DD/MM/YYYY)',                                                        category: 'Employment', inputType: 'Date',       required: 'Mandatory',   helpText: 'Include full dates (DD-MM-YYYY). Include part-time, full-time, foreign work experience, and self-employment (Uber, Skip, DoorDash, etc.).' },
  ...
  // ── Section 3: Travel History (Past 5 Years) ─────────────────────────────
  { name: 'Travel – Start Date (DD/MM/YYYY)',                                                            category: 'Travel',     inputType: 'Date',       required: 'Mandatory',   helpText: 'Provide travel history for the past 5 years (including home country). Include full dates (DD-MM-YYYY) and your status (Student, Visitor, Worker, Citizen, etc.).' },
  ...
  { name: 'Parents/Spouse – Full Name (As per Passport)',                                                category: 'Personal',   inputType: 'Short Text', required: 'Mandatory',   helpText: 'Provide details for father, mother, wife, or husband. If deceased, specify date and city/town of death. If in Canada, include their immigration status and address.' },
  ...
  { name: 'Siblings – Full Name (As per Passport)',                                                      category: 'Personal',   inputType: 'Short Text', required: 'Mandatory',   helpText: 'Include brothers and sisters (including half and stepbrothers/sisters). If deceased, specify date and city/town of death. If in Canada, include immigration status and address.' },
  ...
  { name: 'Refusal Details – Date of Refusal',                                                                                                                            category: 'Legal',      inputType: 'Date',       required: 'Conditional', helpText: 'Required if any refusal or denial question was answered Yes.' },
