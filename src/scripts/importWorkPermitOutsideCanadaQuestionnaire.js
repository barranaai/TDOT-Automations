/**
 * Bulk import Work Permit Outside Canada (SOWP – LMIA) questionnaire into the
 * existing SOWP and LMIA groups, using Case Sub Type = "Outside Canada" to
 * distinguish from the Inside Canada question set.
 *
 * Usage: node src/scripts/importWorkPermitOutsideCanadaQuestionnaire.js
 */
...
// Target groups (same groups as Inside Canada, differentiated by Case Sub Type)
const TARGET_GROUPS = [
  { groupId: 'group_mm12xza9', caseType: 'SOWP', prefix: 'SOWP-OC' },
  { groupId: 'group_mm12v89',  caseType: 'LMIA', prefix: 'LMIA-OC' },
];

// ─── Main Applicant Questions ────────────────────────────────────────────────
...
const MAIN_QUESTIONS = [
  ...
  { name: 'Other Name – Given Name',                                                         category: 'Personal',   inputType: 'Short Text', required: 'Conditional', helpText: 'Required if you have used another name.' },
  ...
  { name: 'Education – Start Date (DD/MM/YYYY)',                                             category: 'Education',  inputType: 'Date',       required: 'Mandatory',   helpText: 'Include full dates (DD-MM-YYYY) for each period. Provide details of your education including College, University or any apprentice training.' },
...
// ─── Dependent Applicant Questions ───────────────────────────────────────────
// Same question set as Main Applicant but:
//   - First/Latest Entry to Canada and Last IRCC Application fields are excluded
//   - Question names have " (Dependent)" suffix

const DEPENDENT_EXCLUSIONS = new Set([
  'First Entry to Canada – Date (DD/MM/YYYY)',
  'First Entry to Canada – Location',
  'First Entry to Canada – Status',
  'Latest Entry to Canada – Date (DD/MM/YYYY)',
  'Latest Entry to Canada – Location',
  'Latest Entry to Canada – Status',
  'Last Application to IRCC – Submission Date',
  'Last Application to IRCC – Application Type',
  'Last Application to IRCC – Result (Approved / Refused)',
]);

const DEPENDENT_QUESTIONS = MAIN_QUESTIONS
  .filter((q) => !DEPENDENT_EXCLUSIONS.has(q.name))
  .map((q) => ({ ...q, name: `${q.name} (Dependent)` }));

// Combined list: main first, then dependent
const ALL_QUESTIONS = [...MAIN_QUESTIONS, ...DEPENDENT_QUESTIONS];
...
async function main() {
  ...
  console.log(`Importing ${ALL_QUESTIONS.length} questions (${MAIN_QUESTIONS.length} main + ${DEPENDENT_QUESTIONS.length} dependent)`);
  ...
  for (const group of TARGET_GROUPS) {
    console.log(`\n━━━ ${group.caseType} – Outside Canada (${group.prefix}) ━━━`);
    let groupCreated = 0;
