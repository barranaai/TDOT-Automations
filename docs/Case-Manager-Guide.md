...
| Execution Boards | Monday.com — Document & Questionnaire Boards | One row per document or question per case. Updated as clients submit. |
...
|---|---|---|
| Item Title | Client's full legal name | Used as the folder name in OneDrive and in all emails |
...
| Case Manager | Assign yourself or the lead officer | Used for notifications and escalation routing |
...
| Escalation Required | Set to Yes | If you identify an escalation need that the system has not flagged automatically (e.g., legal complication, complaint). Set to No once resolved — clears the escalation reason automatically. |
...
| Column | Set by |
|---|---|
| Case Reference Number | Fires automatically when Case Type is set |
| Access Token | Generated automatically when the item is created |
| Q Readiness % | Daily readiness scan |
| Doc Readiness % | Daily readiness scan |
| Q Completion Status | Daily readiness scan and questionnaire submission |
...
| Checklist Template Applied | Checklist service on Document Collection Started |
| Q Template Applied | Questionnaire service on Document Collection Started |
| Stage Start Date | Resets automatically on every stage advance |
| Payment Date | Set automatically when Retainer is marked Paid |
| Chasing Stage | Chasing loop (Pending → Reminder 1 → Reminder 2 → Final Notice → Client Blocked → Resolved) |
...
| Expiry Risk Flag | SLA engine and Expiry engine |
| Expiry Alert Sent | Expiry engine — tracks whether a warning or critical email has been sent |
...
| Escalation Reason | Stage gate, expiry engine, and escalation routing |
| Sub-Type Hint | Internal helper for case reference format — do not edit |
...
### When Retainer is Marked Paid
- Case Stage is set to Document Collection Started
- Document checklist rows are created on the Execution Board
- Questionnaire is set up
- Intake email is sent to the client
...
| Engine | What it does |
|---|---|
| Readiness Scan | Counts documents and questionnaire answers. Updates Q Readiness %, Doc Readiness %, and Blocking Doc Count on the Client Master Board. |
| Stage Gates | If readiness meets the threshold (typically 80%) with zero blocking items, the case automatically advances from Document Collection Started to Internal Review, or from Internal Review to Submission Preparation. |
| Chasing Loop | If a client has not responded in a set number of days (configured per case type in the SLA Config board), reminder emails are sent automatically. Escalates through: Reminder 1 → Reminder 2 → Final Notice → Client Blocked. |
| SLA Engine | Calculates days elapsed, expected deadlines, and SLA Risk Band (Green / Orange / Red). Updates SLA columns on the board. |
...
| Q Completion Status | Working on it (in progress) or Done (submitted) |
| Doc Readiness % | Percentage of required documents received and reviewed |
| Doc Threshold Met | Yes when doc readiness meets the threshold — green light for stage advance |
...
| Missing Required | Number of required items (docs and questions) outstanding |
...
- Q Readiness % is at or above the threshold, AND
- Doc Readiness % is at or above the threshold, AND
- Blocking Doc Count is 0
...
| Blocking Document | Yes or No — whether missing this document blocks the stage gate |
...
> Tip: You do not need to chase the client manually. The chasing loop sends reminder emails automatically based on inactivity days. See Section 9.
...
You can also add it as a button or link in the Monday.com item. The first time you open it, you will be asked to log in with your Monday.com account.
...
2. Type your comment or instruction in the note box that appears.
3. Click Save Flag.
...
Once you have flagged all the items that need attention:
...
Click the Export PDF button (⬇) in the toolbar. A new tab opens with a clean, printable report showing:
...
Use your browser's File → Print → Save as PDF, or click the green Save as PDF / Print button on the page.
...
|---|---|---|
| Submission Ready | When you have reviewed everything and the case is ready for the application to be filed | Locks the case — no more automatic stage advances |
| Submitted | After the application has been submitted to immigration authorities | Locks all automation — chasing, SLA, health, and escalation all stop |
...
> - Sets Automation Lock to Yes — all daily engines skip this case
> - Sets Chasing Stage to Resolved — stops all client reminder emails
...
| SLA Risk Band | Green / Orange / Red — overall time risk |
| Hard Deadline / Soft Deadline | Calculated target dates |

### Case Health Status

Separate from SLA. Reflects the overall health of the case based on multiple signals: SLA risk, client responsiveness, inactivity, and blocking items. Values are:

- Green — on track
- Orange — requires attention
- Red — critical, case manager notified immediately

When Case Health turns Red, you receive a Monday.com notification automatically.
...
|---|---|
| (blank) or Pending | Not started — client has not been chased yet |
...
| Resolved | Client has responded or the case has moved forward — chasing stopped |
...
> Use this for situations like: client is travelling, client is waiting on third-party documents, or client has spoken to you directly.
...
| Case Health turns Red | Case Manager |
| Expiry date approaching (passport, IELTS, medical) | Case Manager and Ops Supervisor |
| Escalation Required is set to Yes | Ops Supervisor and Case Manager |
...
### Client has not received their links
Check that the Client Email column is correct and the Access Token column is not empty. If the token is missing, the links will show "Access Denied" for the client. Contact your system admin to trigger a manual resend.

### Case should not advance automatically
Set Automation Lock to Yes. This stops all daily engines from processing the case. Remember to set it back to No when you want automation to resume.

### Client says their form is empty when they open it
The client must use the same link from the original intake email — it contains their security token. Sharing the plain URL without the `?t=...` token will show Access Denied.

### You need to re-send the questionnaire or documents link
...
The access token is visible in the Access Token column on the Client Master Board. Copy it and append it to the questionnaire URL as `?t=` before sending.
...
| From | To | Condition |
|---|---|---|
| Document Collection Started | Internal Review | Q% at or above threshold AND Doc% at or above threshold AND Blocking Docs = 0 |
...
|---|---|---|
| Manual Override = Yes | Chasing emails only | Client has a valid reason for delay — do not want to chase them but the case is still active |
...
- If % consumed reaches the Orange Threshold → SLA Risk Band turns Orange
- If % consumed reaches the Red Threshold → SLA Risk Band turns Red
- Hard and Soft deadlines are calculated as: Stage Start Date + (SLA Total Days − offset)
...
- Q Readiness % is at or above this value, AND
- Doc Readiness % is at or above this value, AND
- Blocking Doc Count is 0
...
> If an expiry date (passport, IELTS, or medical) is also within the warning window, the risk band is forced up regardless of where the SLA time stands — expiry risk always overrides.
...
If the client still has not responded after Reminder 1, a second, firmer reminder is sent after this many total days of inactivity.
...
- Sets Chasing Stage to Client Blocked
- Sets Escalation Required to Yes
- Notifies the Ops Supervisor

This is the automatic escalation trigger for non-responsive clients.

> The four chasing offsets work as a ladder: Reminder 1 → Reminder 2 → Final Notice → Escalation. Each fires only once per period of inactivity. If the client responds at any point, the chain resets.
...
| Min Threshold | Readiness Engine and Stage Gate |
| Orange / Red Threshold | SLA Risk Engine |
| Hard / Soft Deadline Offset | SLA Risk Engine |
| Expiry Warning Days | SLA Risk Engine and Expiry Engine |
...
> Never rename a row. The row name must exactly match the Case Type dropdown value on the Client Master Board. If they do not match, the automation falls back to system defaults for that case type and no warning is shown.
